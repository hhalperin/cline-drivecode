...
Desktop host (Tauri)
- Kanban has no desktop shell of its own. It had an Electron one under `packages/desktop`; that was deleted when the repo consolidated onto the Tauri app at `apps/examples/desktop-app`, which already carries signing, notarization and the auto-update feed. Kanban now ships two packages instead: `packages/desktop-bridge` (host-agnostic policy — update transitions, notification dedupe, badge rules, payload schemas) and `packages/desktop-tauri` (the adapter). `@tauri-apps/api` and `@tauri-apps/plugin-notification` are imported in exactly one file, `desktop-tauri/src/real-surface.ts`. Keep it that way for the same reason `src/cline-sdk/` exists.
- The bridge is mounted at `web-ui/src/desktop/desktop-provider.tsx`, which is the composition root and the only production caller of `createDesktopBridge` / `createTauriDesktopHost`. It assigns `window.desktop` and exposes `useDesktop()`. Worth knowing that it did not exist for a while: the contract, adapter, controllers and ~190 tests all shipped before anything called them, so every namespace was inert in the running product no matter what the host advertised — and nothing failed, because absent capabilities are documented no-ops. If you are wondering why a desktop feature does nothing, check that the handshake actually ran before you go looking at the host.
- `useDesktop()` returns `null` in a browser tab, which is the ordinary case rather than an error. Branch on it; do not assert. And prefer `desktop?.capabilities.includes("windows")` over a platform check — a host can be present and still not implement a namespace. Note `DesktopApi` exposes `capabilities` as an array, not a `has()` method.
...
- The declared/derived split has a practical consequence worth knowing before you go looking: wiring `updates` and `notifications` needed **no change to `kanban.rs`'s `CAPABILITIES`**, because `createDesktopBridge` derives those from the objects the host supplies. What it did need was the plugin linked in `Cargo.toml` *and* the matching permission in **every** capability manifest — plugin commands are permission-gated even though this app's own commands are not, so `notification:default` has to name the project windows or the call is denied exactly where Kanban runs.
- Kanban's updater deliberately does **not** drive `@tauri-apps/plugin-updater`. `main.rs` already checks and installs on a two-hour loop, so a second updater would be two writers racing to replace one bundle — the same two-owners-one-resource shape ADR-0036 records for the tray, with a worse failure mode. `desktop-tauri/src/updater-backend.ts` reads the host's status and asks it to check; `install()` means "restart to apply", which is the only step a user is actually left. The host reports no download percentage, so `downloading` maps to `available` rather than inventing one.
- `tauri::test` (`mock_builder` + `get_ipc_response`, behind a `test` feature in dev-dependencies) runs a command through the real IPC path with no display. That is the layer where an async command's machinery lives — the generated wrapper, `State<'_, T>` held across an await — and calling the inner function directly exercises none of it. A command has to be generic over `R: tauri::Runtime` to be reachable that way; pinning it to `AppHandle` (i.e. `Wry`) is what makes it untestable.
...
- **Bundling needs more than compiling does**, and the error lands in a confusing place. Those four libraries are enough for `cargo check` and `cargo test`, and the release binary links fine — then `tauri build` aborts with `Can't detect any appindicator library`. The crate builds with tauri's `tray-icon` feature and the *bundler* resolves it at package time, so everything that looks like it would fail has already succeeded by then. Add `libayatana-appindicator3-dev` and `libxdo-dev` before a packaging run. `desktop-test.yml` installs all six for its `packaged` job and only the four for the fast `rust` job, which is the distinction in a nutshell.
- The tray is decided (ADR-0036, Accepted): Kanban publishes a presence summary and the **host** places it, in its own item next to Cline's `set_tray_status`-owned "N sessions running". Kanban never owns a tray item — N project windows each mount their own `PresenceController` and will each publish, and only the host can see all of them to merge. What it is *not* is a scarce menu row, which is how this was first written down; `TrayMenuState` is a struct of items and the tray already runs two independent write policies.
- `presence` is fed by `useDesktopPresence` (`web-ui/src/desktop/use-desktop-presence.ts`), called from `App` — the producer for the whole namespace. `PresenceController.update` is the only path to the badge, the attention signal *and* the tray summary, and it is reached only from `presence.setCounts`, so if a presence signal looks broken, check that hook before suspecting the host. Counts come from **task-session state** (`running`, `awaiting_review`), never from board columns: `running` becomes an OS wake lock, so it has to mean an agent is executing, not that a card is sitting in `in_progress`. The badge, attention signal and wake lock are live; the tray summary still short-circuits on `hasTray`. Adding `"tray"` to `kanban.rs`'s `CAPABILITIES` is the one-way step and goes last — the host command `kanban_set_tray_summary` first, then the capability (ADR-0036 steps 2–3).

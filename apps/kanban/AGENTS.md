...
- **The entire `presence` namespace is dead code today**, and this catches people out because it looks wired. `PresenceController.update` is the only path to the badge, the attention signal *and* the tray summary, and it is reached only from `presence.setCounts` — which has no production caller. Since #238 mounted the bridge, `window.desktop` exists and `useDesktop()` is exported, but no component calls it. So before assuming a presence signal is broken, check whether anything is feeding it. Adding `"tray"` to `kanban.rs`'s `CAPABILITIES` is the one-way step and goes last: producer, then `kanban_set_tray_summary` in the host, then the capability.

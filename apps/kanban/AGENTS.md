...
- The tray's "N sessions running" item belongs to Cline's own `set_tray_status`, and Kanban's presence summary is the same kind of information about a different subsystem. The blocker is **not** a scarce menu row, which is how this was first written down: `TrayMenuState` is a struct of items and the tray already runs two independent write policies, so a second item costs a field and an `.item()` call. It is that advertising `tray` in `kanban.rs`'s `CAPABILITIES` starts the calls and cannot be withdrawn without breaking the bridge contract — so the contract's shape (does Kanban publish a summary the host places, or own an item outright?) has to be settled before the presentation is. ADR-0036 has it, and turns on ADR-0033 being ratified. Kanban's presence drives only the dock badge and attention signal meanwhile, which are per-window.

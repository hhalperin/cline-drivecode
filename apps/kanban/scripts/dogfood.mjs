...
const cleanupOwnerLockPath = resolve(
	tmpdir(),
	"kanban-dogfood-cleanup-owner.lock",
);
...
async function main() {
	...
	if (skipShutdownCleanup) {
		const ownerPidLabel =
			typeof cleanupOwnership.ownerPid === "number"
				...
				: "";
		console.log(
			`[dogfood] Cleanup owner already active${ownerPidLabel}; this run will skip shutdown cleanup.`,
		);
	} else {
		console.log(
		...
		console.log(
			"[dogfood] This run owns shutdown cleanup and will perform it on exit.",
		);
	}
	...
	try {
		if (!args.skipBuild) {
			...
			const buildCode = await runCommand(npmBinary, ["run", "build"], {
				cwd: repoRoot,
				env: process.env,
			});
			if (buildCode !== 0) {
		...
		if (args.project) {
			...
			console.log(
				`[dogfood] No --project provided; launching from non-git cwd ${launchCwd}`,
			);
			console.log(
				"[dogfood] Kanban will open the first indexed project if one exists.",
			);
		}

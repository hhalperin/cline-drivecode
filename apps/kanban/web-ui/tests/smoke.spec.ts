...
test("creating and opening a backlog task shows the inline editor", async ({
	page,
}) => {
	await page.goto("/");
	...
	await expect(page.getByPlaceholder("Describe the task")).toHaveValue(
		taskTitle,
	);
...
test("escape key closes the backlog inline editor", async ({ page }) => {
	...
	await expect(page.getByText("Backlog", { exact: true })).toBeVisible();
	await expect(
		page.locator("[data-task-id]").filter({ hasText: taskTitle }).first(),
	).toBeVisible();
});
...
test("settings button opens runtime settings dialog", async ({ page }) => {
	...
	await expect(
		page.getByRole("dialog").getByText("Settings", { exact: true }),
	).toBeVisible();
});

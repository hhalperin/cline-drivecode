 ...
 *
 * Plan membership and progressive ids are **declared**, never derived. The
 * phase arrays below are the structure `PLAN_DEPENDENCY_DEMO_ANNOTATIONS` is
 * built from, so a task belongs to exactly the plan it is listed under and to
 * no other. Nothing here reads a `description` string: those read
 * `"Phase 1 · DRV-ROSTER"` and scraping them would let the map invent
 * structure out of task prose, which
 * `initiatives/status-dependency-graph/UX.md` ("Membership source", "Do not
 * invent progressive IDs in the view from titles") forbids.
 */
 ...
 */
const PLAN_GROUPS: ReadonlyArray<{ title: string; tasks: TeamTask[] }> = [
	{ title: "Kernel, schemas and privacy", tasks: phase0Tasks },
	{ title: "Room and partner MVP", tasks: phase1Tasks },
...
/**
 * `P001` / `T001` … — the progressive-id shape from UX.md, three-digit padded.
 * A fixture stands in for the task bank's counter, so the ids are minted from
 * declaration order here rather than read off anything the map can see.
 */
function progressiveId(prefix: "P" | "T", index: number): string {
	return `${prefix}${String(index + 1).padStart(3, "0")}`;
}

/**
 * Annotations for the dependency map's Plans rail under `?demoPlans=1`.
 *
 * Production has no equivalent: `TeamTask` carries neither plan membership nor
 * a minted id, so the live adapter returns `null` and the rail shows its empty
 * state. That asymmetry is the point — the demo is allowed to declare
 * structure, the projection is never allowed to guess it.
 */
export const PLAN_DEPENDENCY_DEMO_ANNOTATIONS: DependencyMapAnnotations = {
	displayIds: Object.fromEntries(
		planTasks.map((entry, index) => [entry.id, progressiveId("T", index)]),
	),
	plans: PLAN_GROUPS.map((group, index) => ({
		id: progressiveId("P", index),
		displayId: progressiveId("P", index),
		title: group.title,
		taskIds: group.tasks.map((entry) => entry.id),
	})),
};

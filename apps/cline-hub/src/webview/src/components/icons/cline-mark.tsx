/**
 * The Cline bot-head mark as an inline icon.
 *
 * Traced from `public/cline-logo-filled.svg` (the same asset the hub rail
 * logo button renders via `<img>`), but with an explicit `currentColor` fill
 * instead of the original's implicit black + `dark:invert` hack, so it can
 * pick up avatar/theme colors directly like `DriveMarkIcon` does.
 */

...
export class QueuedRateLimiter {
	...
	constructor(config: Partial<RateLimiterConfig> = {}) {
		...
		if (this.config.maxPerInterval < 1) {
			throw new Error("QueuedRateLimiter maxPerInterval must be >= 1");
		}
	...
	async acquire(signal?: AbortSignal): Promise<void> {
		if (this.#closed) {
			throw new Error("QueuedRateLimiter is closed");
		}
		if (signal?.aborted) {
			throw new Error("QueuedRateLimiter acquire aborted");
		}
		if (this.#tokens > 0) {
			this.#tokens -= 1;
			return;
		...
		return new Promise<void>((resolve, reject) => {
			...
			const onAbort = () => {
				...
				reject(new Error("QueuedRateLimiter acquire aborted"));
			};
	...
	close(): void {
		...
		for (const waiter of pending) {
			waiter.reject(new Error("QueuedRateLimiter is closed"));
		}
	}

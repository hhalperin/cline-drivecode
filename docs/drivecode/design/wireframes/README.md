...
| [overview-canvas.html](../canvases/overview-canvas.html) | In-repo replacement for the old Windows-only `cline-drivecode-overview.canvas.tsx` |
...
| [mobile-drive-ios.html](mobile-drive-ios.html) | **2026 frontier phone shell** — light-first; full-bleed Spotlight; liquid-glass hold/tab; Open / Home / Call / Approval / Settings ([mobile-consumer](../../plans/cline-drivemode/initiatives/mobile-consumer/)) |
...
| [mobile-drive-power.html](mobile-drive-power.html) | **Power cockpit** — Live stack, power sheet (task lines / stop / spend), gate blast radius ([POWER-USERS](../../plans/cline-drivemode/initiatives/mobile-consumer/POWER-USERS.md) · [ROADMAP](../../plans/cline-drivemode/initiatives/mobile-consumer/ROADMAP.md)). Asset: [mobile-drive-power-wireframe.png](../../assets/hub/mobile-drive-power-wireframe.png) |
...
Both prototypes are styled from [CLINE-BRAND-TOKENS.md](../brand/CLINE-BRAND-TOKENS.md), measured off cline.bot rather than guessed. Short version: violet `#9F58FA` on near-black `#0A0A0A` surfaces, hairline `0.8px` borders, `9px` radius, DM Sans for display, Space Grotesk for eyebrows and code. UI text is **Schibsted Grotesk**, because that is what the hub webview actually sets (`--font-sans` in `apps/cline-hub/src/webview/src/index.css`) — the site's Inter loses to product fidelity here. Discord blurple is deliberately absent — the Discord reference is information architecture, not palette.

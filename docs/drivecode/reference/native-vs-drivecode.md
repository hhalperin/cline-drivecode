...
|---|---|---|---|---|
| Interaction model | Turn chat: prompt → wait → transcript | Call room: join, roster, stay in the call | `branch` | [00-vision.md](../plans/cline-drivemode/foundation/00-vision.md); [DriveCallChrome.tsx](../../../apps/cline-hub/src/webview/src/drive/DriveCallChrome.tsx) |
...
| Privacy | Session storage norms | Privacy-strict: no transcript/audio persist without explicit debug flag | `plan` | [DRV-PRIVACY](../plans/cline-drivemode/features/DRV-PRIVACY.md); [HANDOFF.md](../plans/cline-drivemode/delivery/HANDOFF.md) |
| Collaboration primitive | Session | Room: participants, stage sharer, addressSet | `branch` | [01-architecture.md](../plans/cline-drivemode/foundation/01-architecture.md) D3; hub `call_*` / `drive.*` |
| Host portability | Cline SDK only | `@cline/drive` host port + capability descriptor + conformance kit | `branch` | [02-architecture.md](../plans/drivecode-sdk/foundation/02-architecture.md); [DRV-KERNEL](../plans/cline-drivemode/features/DRV-KERNEL.md); `sdk/packages/drive` |
| Surface IA | Chat / CLI / IDE extension | Drive mode in Chat (+ optional Drive hub activity + Status Hub) | `branch` | [PRD 8](../plans/cline-drivemode/prd/prd-drive-as-cline-mode.md); [DRIVE-TAB.md](../design/wireframes/DRIVE-TAB.md) |
| Media | N/A for coding-agent MVP | Events-first stage (typed work cards); WebRTC pixels later | `branch` | [00-vision.md](../plans/cline-drivemode/foundation/00-vision.md); [02-research-streaming.md](../plans/cline-drivemode/research/02-research-streaming.md) |

...
Phase numbering aligns with [`../cline-drivemode/TASK-GRAPH.md`](../../cline-drivemode/delivery/TASK-GRAPH.md); this document adds the harness and port work to those phases rather than running a parallel track.
...
**Done when** the six documents under `docs/drivecode/plans/drivecode-sdk/` exist, the merge-vs-separate question is resolved with the rejected alternatives written down, and `../cline-drivemode/README.md` points at this directory.
...
10. **`createDriveHarness({ host })` exists** with rooms MVP (`createOrAttach`, `setAddress`, `setSharer`, …) and `director` pure helpers — see [06-sdk-leverage.md](../delivery/06-sdk-leverage.md).

...
- `docs/drivecode/plans/cline-drivemode/foundation/00-vision.md` — naming, "share screen = structured state not pixels", phases.
...
- `docs/drivecode/reference/architecture.md`, `docs/drivecode/reference/native-vs-drivecode.md`, `docs/drivecode/reference/skills-inventory.md`.
- `docs/drivecode/design/wireframes/DEMO.md` — **runnable demo runbook** for the share-screen Join path.
- `docs/drivecode/design/wireframes/DRIVE-TAB.md`, `CLINE-BRAND-TOKENS.md`, `overview-canvas.html`, `drive-tab-discord-slack.html`, `index.html`.
...
- GUI/screenshots: a virtual X display is live at `DISPLAY=:1`. Launch GUI/dev in `tmux`. Product screenshots live in `docs/drivecode/assets/` (e.g. `drive-call.png`, `drive-tab.png`, `status-*.png`).
- Hub webview tests: `bun --cwd apps/cline-hub test` (vitest) for `drive/*` reducers/components.

## Brand / visual reference (for the canvas)

Match the existing self-contained HTML canvas style at `docs/drivecode/design/canvases/overview-canvas.html`:
dark bg `#0A0A0A`, brand purple `#9F58FA`, fonts DM Sans / Schibsted Grotesk / Space Grotesk. Tokens in
`docs/drivecode/design/brand/CLINE-BRAND-TOKENS.md`.

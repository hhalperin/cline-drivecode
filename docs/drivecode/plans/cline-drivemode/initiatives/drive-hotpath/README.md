...
| 3 | In-process agent→stage projector (drop critical-path `call_record_work` RTT) | Daemon `session-event-projector` calls `handleDriveRoomCommand(call_record_work)` after `tool.finished`; remove Hub Chat `recordDriveWorkFromTool` hop atomically | todo — keep command API; cache tool input on content_start; teammate include/exclude is an explicit choice |

...
Decision record and throwaway prototype. [DRIVE-TAB.md](../../../design/wireframes/DRIVE-TAB.md), [drive-tab-discord-slack.html](../../../design/wireframes/drive-tab-discord-slack.html).
...
The screen is structured work state, not pixels. The partner publishes typed events for edits, commands, test runs, plan progress, and decisions. The UI renders them with the card components Cline already bundles. This is cheaper, searchable, privacy-clean, and honest about what an agent actually does. Pixel capture is an explicit anti-pattern for the MVP agent stage (see [02-research-streaming.md](../research/02-research-streaming.md)). User share may start as structured selection share; WebRTC pixels stay later ([04-future-multi-user.md](../research/04-future-multi-user.md)).

## The staged wireframe (locked recommendation)

Primary chrome is the Drive tab. Call Stage and voice still stage in phases. Wireframes. [docs/drivecode/design/wireframes/](../../../design/wireframes/).
...
One developer and one pair partner in the MVP roster. The UI still shows the roster shape so multi-agent and multi-human do not require a chrome rewrite. Teams of agents and multiple humans are a future state ([04-future-multi-user.md](../research/04-future-multi-user.md)). Nothing in the MVP may block that future, and nothing in the MVP builds the multi-human media plane.

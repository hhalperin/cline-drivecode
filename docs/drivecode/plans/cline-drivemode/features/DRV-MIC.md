...
- [ ] Read `speech-input.tsx` to establish what STT engine it assumes (Web Speech, MediaRecorder upload, or pluggable) and document the local-only constraint fit.
  ...
  - Verify: written finding. Selection is via `providers.sttId` + `createVoiceStack` ([ADR-0010](../adr/ADR-0010-provider-harness-byok.md)); Local profile forbids Web Speech ([ADR-0009](../adr/ADR-0009-runtime-topology-local-cloud.md)).

...
- Event families cover the five tracks: control (join/leave/mute/stage/mode), conversation (message, narration), work (edit, command, test result, plan step, decision), presence (speaking, typing, status), media (`media.artifact` — a bytes-free reference to a produced artifact; see #168).
...
- Over-modeling events for multi-user before the MVP proves shapes. Mitigation. Version field plus a media track kept minimal. `media.artifact` landed ahead of its consumer (the Artifacts page); it carries references only, never bytes.

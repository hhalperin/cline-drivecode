...
Back to [README](../../../design/wireframes/README.md). This is a desired-state design, not an MVP work item. Nothing here is built until the join-call MVP is proven. Everything in the MVP must be compatible with it.
...
The MVP Drive tab already ships this IA for one workspace and one partner (see [DRIVE-TAB.md](../../../design/wireframes/DRIVE-TAB.md)). Multi-user extends seats, remote bridging, and media. It does not replace the tab.
...
All three default `false` on `CLINE_HOST_CAPABILITIES` until implemented. See [LEADERSHIP-BRIEF.md](../leadership/LEADERSHIP-BRIEF.md) and ops [hub-drive-ops.md](../ops/hub-drive-ops.md).

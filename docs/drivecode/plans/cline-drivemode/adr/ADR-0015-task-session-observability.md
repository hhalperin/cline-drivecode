# ADR-0015: Local task-session observability; tasks as satisfaction unit
...
7. **Closed-loop improvement is gated.** Stall diagnosis uses structured event ids / paths / skill ids only. Proposals (planning skills, plan templates, fix-up tasks) follow propose → accept | reject | mute (ADR-0004). No auto transcript→knowledge; no silent plan rewrite by a model.
...
- [ADR-0004](ADR-0004-gated-learn-privacy.md), [ADR-0008](ADR-0008-task-bank.md)

...
| [ADR-0036](ADR-0036-desktop-tray-ownership.md) | Desktop tray ownership (Cline vs Kanban) | **Accepted** | partial | Kanban publishes a summary, host places it; separate item, not a merged line. Producer landed (`useDesktopPresence`) so badge/attention/wake-lock are live; host command + `tray` capability remain |

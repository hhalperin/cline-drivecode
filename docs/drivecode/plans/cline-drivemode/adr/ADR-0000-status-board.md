...
| [ADR-0036](ADR-0036-desktop-tray-ownership.md) | Desktop tray ownership (Cline vs Kanban) | **Accepted** | none | Kanban publishes a summary, host places it; separate item, not a merged line. Sequenced behind a `presence.setCounts` producer — that namespace has none |

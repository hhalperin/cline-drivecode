...
When teams are enabled, the coordinator agent gets additional tools (see `TEAM_TOOL_NAMES` in `@cline/core`):

| Tool | Description |
|------|-------------|
| `team_spawn_teammate` | Create a new agent with a specific role |
| `team_task` / `team_run_task` | Assign / run a task for a teammate |
| `team_status` | Check teammate / run status |
| `team_await_runs` / `team_list_runs` | Wait for or list completed runs |
| `team_send_message` | Send a message to a teammate |

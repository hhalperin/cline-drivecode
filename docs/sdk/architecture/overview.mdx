...
CLI, Hub, and Desktop use Core MCP settings-file tooling. VS Code sets `disableMcpSettingsTools: true` and injects tools through its legacy `McpHub` + `createMcpTools`. Treat that split as intentional — [SDK-7.2](/sdk/architecture/vscode-mcp-spike) recommends keeping the adapter.

...
When `automation` is enabled, use `cline.automation` to start/stop the embedder cron service, `reconcileNow`, ingest events, and list events/specs/runs. Product CLI/Hub/Desktop schedules use Hub `schedule.*` instead — see [Scheduled agents](/sdk/guides/scheduled-agents).
